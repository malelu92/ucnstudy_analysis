...
def get_devtraffic_data():
    ...
    for user in users:
        ...
        for dev in user_devices:
            ...
            sql_beg_day = text('SELECT distinct devid, devicetraffic.ts \
            FROM devicetraffic join \
            (SELECT DATE(ts) as date_entered, MIN(ts) as min_time \
            FROM devicetraffic \
            WHERE devid =:d_id and extract (hour from ts) > 3 \
            GROUP BY date(ts)) AS grp ON grp.min_time = devicetraffic.ts order by devicetraffic.ts;').bindparams(d_id = dev.device_id)
            result_beg_day = ses.execute(sql_beg_day)
            sql_end_day = text('SELECT distinct devid, devicetraffic.ts \
            FROM devicetraffic join \
            (SELECT DATE(ts) as date_entered, MAX(ts) as max_time \
            FROM devicetraffic \
            WHERE devid = :d_id \
            GROUP BY date(ts)) AS grp ON grp.max_time = devicetraffic.ts order by devicetraffic.ts;').bindparams(d_id = dev.device_id)
            result_end_day = ses.execute(sql_end_day)
            sql_beg_day_nolimit = text('SELECT distinct devid, devicetraffic.ts \
            FROM devicetraffic join \
            (SELECT DATE(ts) as date_entered, MIN(ts) as min_time \
            FROM devicetraffic \
            WHERE devid =:d_id \
