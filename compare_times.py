...
def main():
    ...
    cmp_file = open('cmp_devtfc_flow.txt','w') 
    ...
    for user in users:
        ...
        analize_timst(cmp_file, devtfc_week_beg[user.username], flow_week_beg[user.username], 'dev', 'Beg', user, len(devtfc_week_beg[user.username]), 1000)
        analize_timst(cmp_file, devtfc_week_end[user.username], flow_week_end[user.username], 'dev', 'End', user, len(devtfc_week_end[user.username]), 1000)
        cmp_file.write('\n')

    cmp_file.close()

def analize_timst(cmp_file,info_week, activity_week, data_comp, key_beg_end, user, quantity_dev, time_interval):
