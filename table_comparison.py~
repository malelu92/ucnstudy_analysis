...
def main():
    ...
    for user in users:
        ...
        fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(nrows = 2, ncols = 2, figsize=(20, 25))
        ...
        scatter_plot(ax1, devtfc_week_beg[user.username][0], 'beg', 'dev traffic', days_str, user,len(devtfc_week_beg[user.username]))
        scatter_plot(ax2, devtfc_week_end[user.username][0], 'end', 'dev traffic', days_str, user,len(devtfc_week_end[user.username]))
        scatter_plot(ax3, flow_week_beg[user.username][0], 'beg', 'Flow', days_str, user,len(flow_week_beg[user.username]))
        scatter_plot(ax4, flow_week_end[user.username][0], 'end', 'Flow', days_str, user,len(flow_week_end[user.username]))
