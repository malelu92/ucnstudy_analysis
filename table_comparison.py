...
def main():
    ...
    for user in users:
        ...
        fig, ((ax1, ax3, ax5, ax7, ax9), (ax2, ax4, ax6, ax8, ax10)) = plt.subplots(nrows = 2, ncols = 5, figsize=(35, 10))
        ...
        scatter_plot(ax9, flow_week_beg[user.username][0], 'beg', 'Flow', days_str, user,len(flow_week_beg[user.username]))
        scatter_plot(ax10, flow_week_end[user.username][0], 'end', 'Flow', days_str, user,len(flow_week_end[user.username]))
        ...
        fig.savefig('figs_scatter_comparison/' + user.username + '-usage-comparison.png')
        plt.close(fig)
